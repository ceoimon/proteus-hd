...
describe('Session', () => {
  ...
  it('should limit the number of sessions', function (done) {
    ...
    return Proteus.session.Session.init_from_prekey(alice_ident, bob_bundle(1, bob_store))
      ...
      .then(() => {
        assert.isAtMost(alice.session_states.length, Proteus.session.Session.MAX_SESSION_STATES);
        assert.isAtMost(bob.session_states.length, Proteus.session.Session.MAX_SESSION_STATES);
      })
      .then(() => {
        done();
      })
